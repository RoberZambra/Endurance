...
// ── Chave Gemini: lida e salva no localStorage ────────────────────────────────
const GEMINI_KEY_STORAGE = 'endurance_gemini_api_key';
...
function markdownToHTML(text) {
  ...
  // Paleta de cores para números com unidades
  const NUM_COLORS = ['#0047ff','#e85d1a','#0a8a5c','#9333ea','#b45309','#0891b2','#dc2626'];
  let _numColorIdx = 0;
  function nextNumColor() {
    return NUM_COLORS[_numColorIdx++ % NUM_COLORS.length];
  }

  /**
   * Destaca números seguidos de unidades esportivas com um badge colorido
   * Ex: "180 bpm", "42,5 km", "250W", "85%", "1h30min"
   */
  function colorizeNumbers(str) {
    // Reseta índice por seção para cores consistentes
    _numColorIdx = 0;
    return str.replace(
      /(\b\d[\d.,]*\s*(?:bpm|km\/h|km|m\/s|w\b|watts?|kcal|cal|%|min|seg|s\b|h\b|hrs?|horas?|metros?|m\b|kg|lbs?|rpm|mmhg|ml\/kg|vo2|z[1-5]\b))/gi,
      (match) => {
        const color = nextNumColor();
        return `<span class="ai-num" style="color:${color};background:${color}18;border-color:${color}30;">${match}</span>`;
      }
    )
    // Números isolados com sinal + ou - (ex: "+12", "-3") que indiquem variação
    .replace(/(?<![a-z\d])([+\-]\d[\d.,]*)\s*(%|bpm|km|w\b|watts?|kcal|min)?/gi, (match, num, unit) => {
      if (!unit && !/[+\-]/.test(num)) return match;
      const color = num.startsWith('+') ? '#0a8a5c' : '#dc2626';
      return `<span class="ai-num" style="color:${color};background:${color}18;border-color:${color}30;">${match}</span>`;
    });
  }
  ...
  function renderBody(raw) {
    ...
    for (const line of bodyLines) {
      ...
      if (recMatch) {
        ...
        recItems.push(colorizeNumbers(recMatch[2]));
        inRecs = true;
        continue;
      }
      ...
      if (inRecs && recItems.length) {
        const recColors = ['#0047ff','#e85d1a','#0a8a5c','#9333ea','#b45309','#0891b2'];
        out += '<div class="ai-rec-wrap">' + recItems.map((r,i) => {
          const c = recColors[i % recColors.length];
          return `<div class="ai-rec"><div class="ai-rec-num" style="background:${c};">${i+1}</div><div class="ai-rec-text">${r}</div></div>`;
        }).join('') + '</div>';
        recItems = [];
        inRecs = false;
      }
      ...
      if (/^[-•*]\s+/.test(trimmed)) {
        ...
        out += `<li>${colorizeNumbers(trimmed.replace(/^[-•*]\s+/, ''))}</li>`;
        continue;
      }

      if (inUl) { out += '</ul>'; inUl = false; }
      out += `<p>${colorizeNumbers(trimmed)}</p>`;
    }
...
function openApiKeyModal() {
  ...
  if (!modal) {
    ...
    modal.style.cssText = `
      position:fixed;inset:0;background:rgba(10,10,18,.6);backdrop-filter:blur(8px);
      z-index:300;display:flex;align-items:center;justify-content:center;padding:1.5rem;
      animation:fadeIn .18s ease;
    `;
    modal.innerHTML = `
      <div style="background:var(--bg2);border:1.5px solid var(--border);border-radius:var(--rl);
                  width:100%;max-width:420px;position:relative;overflow:hidden;
                  box-shadow:0 28px 60px rgba(0,0,0,0.2);animation:slideUp .2s cubic-bezier(.34,1.4,.64,1);">
        <div style="height:3px;background:linear-gradient(90deg,#7c3aed,#a78bfa);"></div>
        <div style="padding:1.5rem;">
          <button onclick="closeApiKeyModal()" style="position:absolute;top:.78rem;right:.78rem;
            background:var(--bg3);border:1.5px solid var(--border);border-radius:4px;
            color:var(--muted);width:26px;height:26px;display:flex;align-items:center;
            justify-content:center;cursor:pointer;font-size:.85rem;">✕</button>

          <div style="display:flex;align-items:center;gap:.6rem;margin-bottom:1rem;">
            <div style="width:32px;height:32px;background:rgba(124,58,237,.12);border-radius:8px;
            ...
            <div>
              <div style="font-family:var(--font-d);font-weight:700;font-size:1rem;color:var(--text);">
                Chave API do Gemini
              </div>
              <div style="font-family:var(--font-m);font-size:.75rem;color:var(--muted);">
                Necessária para análise com IA
              </div>
            </div>
          </div>

          <p style="font-family:var(--font-m);font-size:.82rem;color:var(--muted);margin-bottom:1rem;line-height:1.55;">
            Obtenha sua chave gratuita em
            <a href="https://aistudio.google.com/app/apikey" target="_blank"
               style="color:#7c3aed;text-decoration:none;font-weight:600;">
              aistudio.google.com
            </a>.
            Ela será salva apenas no seu navegador (localStorage) e nunca enviada a terceiros.
          </p>

          <label style="font-family:var(--font-m);font-size:.78rem;font-weight:600;
          ...
          <div style="position:relative;margin-bottom:.9rem;">
            <input id="apiKeyInput" type="password"
              ...
              style="width:100%;box-sizing:border-box;padding:.6rem 2.4rem .6rem .75rem;
                     font-family:var(--font-m);font-size:.85rem;color:var(--text);
            ...
            />
            <button onclick="toggleApiKeyVisibility()" id="apiKeyToggleBtn"
          ...
          <div id="apiKeyError" style="display:none;font-family:var(--font-m);font-size:.78rem;
            color:#dc2626;margin-bottom:.7rem;padding:.4rem .6rem;background:rgba(220,38,38,.07);
            border:1px solid rgba(220,38,38,.2);border-radius:5px;">
          </div>

          <div style="display:flex;gap:.6rem;justify-content:flex-end;">
            ${getGeminiApiKey() ? `
            <button onclick="clearApiKeyAndClose()"
              style="font-family:var(--font-m);font-size:.82rem;color:#dc2626;
                     background:none;border:1px solid rgba(220,38,38,.25);border-radius:5px;
            ...
            <button onclick="closeApiKeyModal()"
              style="font-family:var(--font-m);font-size:.82rem;color:var(--muted);
                     background:var(--bg3);border:1.5px solid var(--border);border-radius:5px;
                     padding:.45rem .9rem;cursor:pointer;transition:all .15s;">
              Cancelar
            </button>
            <button onclick="confirmApiKey()"
              style="font-family:var(--font-m);font-size:.82rem;color:#fff;font-weight:600;
                     background:#7c3aed;border:none;border-radius:5px;
                     padding:.45rem 1.1rem;cursor:pointer;transition:opacity .15s;"
    ...
    document.getElementById('apiKeyInput').value = getGeminiApiKey();
  }
...
function confirmApiKey() {
  const input  = document.getElementById('apiKeyInput');
  const errDiv = document.getElementById('apiKeyError');
  const key    = input?.value?.trim() || '';

  if (!key) {
    errDiv.textContent = 'Por favor, insira uma chave API válida.';
    errDiv.style.display = 'block';
    input.focus();
    return;
  }
  if (!key.startsWith('AIza') || key.length < 20) {
    errDiv.textContent = 'Chave inválida — deve começar com "AIza" e ter pelo menos 20 caracteres.';
    errDiv.style.display = 'block';
    input.focus();
    return;
  }
  ...
  closeApiKeyModal();
  toast('Chave API salva com sucesso!', 'ok');
  // Dispara a análise automaticamente após salvar
  setTimeout(() => runAIAnalysis(), 200);
}

function clearApiKeyAndClose() {
  clearGeminiApiKey();
  updateApiKeyStatusLabel();
  closeApiKeyModal();
  toast('Chave API removida.', 'warn');
}
...
window.updateApiKeyStatusLabel = updateApiKeyStatusLabel;

window.openApiKeyModal      = openApiKeyModal;
window.closeApiKeyModal     = closeApiKeyModal;
window.toggleApiKeyVisibility = toggleApiKeyVisibility;
window.confirmApiKey        = confirmApiKey;
window.clearApiKeyAndClose  = clearApiKeyAndClose;
